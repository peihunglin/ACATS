...
display_noeol () {
  printf "$@"
  printf "$@" >> $dir/acats-rose.sum
  printf "$@" >> $dir/acats-rose.log
}

display () {
  echo "$@"
  echo "$@" >> $dir/acats-rose.sum
  echo "$@" >> $dir/acats-rose.log
}
...
ROSE="${ROSE-`which testTranslator`}"
...
target_rosemake () {
  if [ "$BASE" ]; then
    ...
    echo testTranslator $gnatflags $gccflags $* -largs $EXTERNAL_OBJECTS
    $ROSE $gnatflags $gccflags $* -largs $EXTERNAL_OBJECTS
    echo $GCC -c $gnatflags $gccflags $dir/support/report.adb  -largs $EXTERNAL_OBJECTS
    $GCC -c $gnatflags $gccflags $dir/support/report.adb  -largs $EXTERNAL_OBJECTS
    echo gnatbind -I$dir/support -x ${3%%.*}.ali
    gnatbind -I$dir/support -x ${3%%.*}.ali
    echo gnatlink ${3%%.*}.ali -g -O2
    gnatlink ${3%%.*}.ali -g -O2
  fi
}
...
run_one_test_rose () {
  ...
  rm -rf $test
  mkdir $test && cd $test >> $dir/acats-rose.log 2>&1
  ...
  echo "BUILD $main" >> $dir/acats-rose.log
  case $tst in
  ...
  target_rosemake $extraflags -I$dir_support $main \
                  > $dir/tests/$chapter/${tst}/${tst}.log 2>&1
  compilation_status=$?
  cat $dir/tests/$chapter/${tst}/${tst}.log >> $dir/acats-rose.log
  ...
  echo "RUN $binmain" >> $dir/acats-rose.log
  cd $dir/run
  ...
  cat ${tst}.log >> $dir/acats-rose.log
...
rm -f $dir/acats-rose.sum $dir/acats-rose.log
...
display `$GCC -v 2>&1`
display target rose is $ROSE
display `$ROSE --version 2>&1`
display host=`gcc -dumpmachine`
display target=$target
display `type gnatmake`
gnatls -v >> $dir/acats-rose.log
display ""
...
if [ -n "$GCC_RUNTEST_PARALLELIZE_DIR" ]; then
  ...
  if [ ! -d $dir_support ] || [ $# -eq 0 ] || [ $1 = "NONE" ]; then
    ...
    target_gnatmake $testdir/support/impbit.adb >> $dir/acats-rose.log 2>&1
    if [ $? -ne 0 ]; then
    ...
    echo target_bit="$target_bit" >> $dir/acats-rose.log
    ...
    echo target_insn="$target_insn" >> $dir/acats-rose.log
    ...
    for i in $dir/support/*.ada $dir/support/*.a; do
      host_gnatchop $i >> $dir/acats-rose.log 2>&1
    done
    ...
    ./macrosub >> $dir/acats-rose.log 2>&1
    ...
    target_gnatmake $testdir/support/send_sigint_to_parent.adb \
                    >> $dir/acats-rose.log 2>&1
    if [ $? -ne 0 ]; then
      display "**** Failed to compile send_sigint_to_parent"
      exit 1
    fi

    target_gcc -c *.c >> $dir/acats-rose.log 2>&1
    if [ $? -ne 0 ]; then
      display "**** Failed to compile C code"
      exit 1
    fi

    target_gnatchop *.adt >> $dir/acats-rose.log 2>&1

    target_gnatmake -c -gnato -gnatE *.ads >> $dir/acats-rose.log 2>&1
    target_gnatmake -c -gnato -gnatE *.adb >> $dir/acats-rose.log 2>&1
